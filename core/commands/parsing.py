...
def extract(PARAMS):
    ...
    for flag_token in flag_tokens:
        ...
        if cleaned == "files":
            ...
            for token in flag_contents:
                ...
                if "*" in value:
                    matches = glob(value, recursive=True)
                    config["files"] += [f for f in matches if Path(f).is_file()]
                else:
                    path = Path(value)
                    if path.is_file():
                        config["files"].append(str(path))
    ...
    # Load template
    template_file = Path(f"data/templates/{config['template']}.json")
    ...
    output_name = config["name"] or config["template"]
    ...
    for flag_token in flag_tokens:
        ...
        if cleaned == "files":
            for token in flag_contents:
                ...
                p = Path(val)
                if p.is_dir():
                    config["files"] += [str(f) for f in p.glob("*.json") if f.is_file()]
                elif "*" in val:
                    config["files"] += [f for f in glob(val, recursive=True) if Path(f).is_file()]
                elif p.is_file():
    ...
    for file_path in config["files"]:
        try:
            ...
            for entry in data:
                ...
                lines = []
                try:
                    source_file = Path("data/ingested") / filename
                ...
                # Label match
                if config["label"]:
                    label_val = entry.get("label", "").lower()
                    for label in config["label"]:
                        if label in label_val:
                            match_types.append(entry.get("label", ""))
                            matches.append({"key": "label", "text": entry.get("label", ""), "line": -1})
                ...
                if config["extract"]:
                    ...
                    for condition in config["extract"]:
                        if "=" in condition:
                            ...
                            actual = extracted.get(key.strip(), {})
                            if isinstance(actual, dict):
                                text_value = actual.get("text", "")
                            else:
                                text_value = str(actual)
                            if val.strip().lower() in text_value.lower():
                                match_types.append(key.strip())
                                match_line = next((i+1 for i, l in enumerate(lines) if val.strip().lower() in l.lower()), -1)
                                matches.append({"key": key.strip(), "text": text_value.strip(), "line": match_line})

                        elif "includes" in condition:
                            key, val = condition.split("includes", 1)
                            key = key.strip()
                            val = val.strip().strip('"')
                            ...
                            if isinstance(actual, dict):
                                ...
                                text_value = str(actual)
                            if val.lower() in text_value.lower():
                                match_types.append(key)
                                match_line = next((i+1 for i, l in enumerate(lines) if val.lower() in l.lower()), -1)
                                matches.append({"key": key, "text": text_value.strip(), "line": match_line})
...
def restore(PARAMS):
    ...
    backup_path = Path("data/memory/backups") / config["name"]
